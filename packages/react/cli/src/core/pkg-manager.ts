...
export interface PackageManager {
  ...
  /** argv to add runtime dependencies, e.g. ["npm","install"] or ["pnpm","add"]. */
  addCommand: string[];
}
...
const NPM_DEFAULT: PackageManager = { name: "npm", addCommand: ["npm", "install"] };
...
export function detectPackageManager(dir: string): PackageManager {
  for (const { file, pm } of LOCKFILES) {
    ...
    if (pm.name === "pnpm" && existsSync(join(dir, "pnpm-workspace.yaml"))) {
