...
import { CREATE_ROOT_RENDER } from "./router-wire";
 ...
 */
export function appImportSpecifier(entrySource: string): string | null {
  ...
  const m = CREATE_ROOT_RENDER.exec(entrySource);
  if (matches.length !== 1 || !m) return null;
