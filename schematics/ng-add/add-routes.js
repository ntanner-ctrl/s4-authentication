 ...
 */
function addAuthRoutes(routesPath) {
    return (tree, context) => {
        if (!routesPath || !tree.exists(routesPath)) {
            context.logger.warn(`s4-auth-angular: no app.routes.ts or app-routing.module.ts found to wire ` +
        ...
        visit(source);
        if (!routesArray) {
            context.logger.warn(`s4-auth-angular: found ${routesPath} but no \`routes\` array literal to ` +
